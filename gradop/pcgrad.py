...
class PCGrad():
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        g_i = pc_grad[0]
        g_j = pc_grad[1]
        g_i_g_j = torch.sum(g_i * g_j, dim=-1, keepdim=True)
        g_i_g_j = g_i_g_j.expand_as(g_i)
        mask_conflict = g_i_g_j < 0

        g_i[mask_conflict] -= g_i_g_j[mask_conflict] * g_j[mask_conflict] / \
            (g_j[mask_conflict].norm(dim=-1, keepdim=True)**2)

        g_j[mask_conflict] -= g_i_g_j[mask_conflict] * g_i[mask_conflict] / \
            (g_i[mask_conflict].norm(dim=-1, keepdim=True)**2)
